...
Because a lot of data is split among different files and therefore read in in different dataframes, they should be combined before starting analysis
```{r}
...
AllRanks %>% nrow()
AllTopTierIDs %>% nrow()
AllTopTierMatches %>% nrow()
```
After combining datasets, we can see that there are 1,872,617 ranked players in NA, 2346 of which are in Master tier or above. Of those players, there were 234,394 games recorded.
